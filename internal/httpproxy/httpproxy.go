...
func (h *Handler) forward(w http.ResponseWriter, r *http.Request, body []byte, observeTool string, id json.RawMessage, jsonRPC bool) {
	...
	outReq, err := http.NewRequestWithContext(r.Context(), r.Method, target, reqBody) // #nosec G107 G704 -- AgentFence is a reverse proxy; building a request to the operator-configured upstream URL is its purpose
	if err != nil {
	...
	resp, err := h.client.Do(outReq) // #nosec G107 G704 -- AgentFence is a reverse proxy; forwarding to the operator-configured upstream is its purpose
	if err != nil {
